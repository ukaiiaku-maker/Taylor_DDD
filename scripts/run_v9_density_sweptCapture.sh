#!/usr/bin/env bash
# v9 density sweep for the Arrhenius Taylor/Peierls DDD harness.
...
DRIVER="${DRIVER:-clean_arrhenius_taylor_ddd_v9.py}"
ROOT="${ROOT:-$(pwd)/results/v9_density_sweptCapture}"
...
mkdir -p "$ROOT"
echo "v9 density sweep T=$T rho=[$RHO_LIST] bs=[$BS_LIST] PeierlsS=$PEIERLS_ENTROPY_KB CrossS=$CROSS_ENTROPY_KB MAX_FREE_DX=$MAX_FREE_DX capture=swept_crossing"
...
echo "Also inspect clean_arrhenius_params.json for v8_peierls_only_baseline_tau_MPa and v9_capture_mode."
