...
def main(argv=None) -> int:
    ...
    # v3: tie the bow geometry to the SAME line tension that makes the reaction force.
    # Over-damped curvature flow  B dx/dt = T d2x/dz2  discretizes to a smoothing step
    # with coefficient relax = 2*T*dt / (B * dz_m^2).  If the user passes
    # --line-tension-relax > 0 we honour it (manual override); otherwise we derive it.
    ...
    if args.line_tension_relax and args.line_tension_relax > 0.0:
        ...
        relax_eff = min(relax_raw, float(args.line_tension_relax_max))
    ...
    with open(outdir / "single_glider_history.csv", "w", newline="") as hf, \
        ...
        for step in range(1, nsteps + 1):
            ...
            # relax_eff is derived from the physical line tension + drag (see above),
            # so the bow shape and the pin reaction force use the SAME line tension.
