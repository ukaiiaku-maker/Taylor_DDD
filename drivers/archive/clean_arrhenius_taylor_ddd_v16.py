...
def write_avalanche_summary(outdir: Path, params: dict, target_strain: float) -> None:
    """Detect bursty / avalanche-like depinning and plastic-strain episodes."""
    hist_path = outdir / "single_glider_history.csv"
    if not hist_path.exists():
        return
    import csv as _csv

    quiet_steps = max(0, int(params.get("avalanche_quiet_steps", 2)))
    window_steps = max(1, int(params.get("avalanche_window_steps", 100)))
    min_events = max(1, int(params.get("avalanche_min_events", 2)))
    plastic_factor = float(params.get("avalanche_plastic_factor", 1.0))
    ...
    def gini(vals):
        vals = sorted([max(0.0, v) for v in vals if math.isfinite(v)])
        if not vals:
            return float("nan")
        s = sum(vals)
        if s <= 0:
    ...
    steps = [_safe_int(r.get("step"), i + 1) for i, r in enumerate(rows)]
    times = [_safe_float(r.get("time_s"), 0.0) for r in rows]
    eps_total = [_safe_float(r.get("eps_total"), 0.0) for r in rows]
    tau = [_safe_float(r.get("tau_MPa"), float("nan")) for r in rows]
    d_tau = [_safe_float(r.get("d_tau_step_MPa"), 0.0) for r in rows]
    d_eps_p = [_safe_float(r.get("d_eps_p"), 0.0) for r in rows]
    d_eps_total = [_safe_float(r.get("d_eps_total"), 0.0) for r in rows]
    n_depin = [_safe_float(r.get("n_depin"), 0.0) for r in rows]
    n_capture = [_safe_float(r.get("n_capture"), 0.0) for r in rows]
    n_live = [_safe_float(r.get("n_live_pins"), 0.0) for r in rows]
    dt = _safe_float(params.get("dt"), float("nan"))
    rate_dt = []
    for r in rows:
        rr = _safe_float(r.get("crossing_rate_max_s"), float("nan"))
        rate_dt.append(rr * dt if math.isfinite(rr) and math.isfinite(dt) else float("nan"))

    med_deps_total = median([v for v in d_eps_total if v > 0])
    if not math.isfinite(med_deps_total) or med_deps_total <= 0:
        med_deps_total = abs(float(params.get("strain_rate", 0.0)) * float(params.get("dt", 0.0)))
    plastic_threshold = plastic_factor * max(med_deps_total, 0.0)

    active = [(nd > 0.0) or (dp > max(plastic_threshold, de)) for nd, dp, de in zip(n_depin, d_eps_p, d_eps_total)]

    avalanches = []
    start = None; last_active = None
    for i, is_active in enumerate(active):
        if is_active:
            if start is None:
                start = i
            last_active = i
        elif start is not None and (i - last_active) > quiet_steps:
            avalanches.append((start, last_active))
            start = None; last_active = None
    if start is not None:
        avalanches.append((start, last_active))

    av_rows = []
    for k, (i0, i1) in enumerate(avalanches, start=1):
        idx = range(i0, i1 + 1)
        size_events = sum(max(0.0, n_depin[i]) for i in idx)
        plastic_size = sum(max(0.0, d_eps_p[i]) for i in idx)
        imposed_size = sum(max(0.0, d_eps_total[i]) for i in idx)
        stress_drop_negsum = -sum(min(0.0, d_tau[i]) for i in idx)
        stress_rise_possum = sum(max(0.0, d_tau[i]) for i in idx)
        tau_seg = [tau[i] for i in idx if math.isfinite(tau[i])]
        peak_to_valley_drop = 0.0
        if len(tau_seg) >= 2:
            running_peak = tau_seg[0]
            for tv in tau_seg:
                running_peak = max(running_peak, tv)
                peak_to_valley_drop = max(peak_to_valley_drop, running_peak - tv)
        rdts = [rate_dt[i] for i in idx if math.isfinite(rate_dt[i])]
        av_rows.append({
            "avalanche_id": k,
            "start_step": steps[i0], "end_step": steps[i1],
            "start_time_s": times[i0], "end_time_s": times[i1],
            "start_eps_total": eps_total[i0], "end_eps_total": eps_total[i1],
            "duration_steps": steps[i1] - steps[i0] + 1,
            "duration_s": times[i1] - times[i0],
            "active_steps": sum(1 for i in idx if active[i]),
            "event_size_depin": size_events,
            "capture_size": sum(max(0.0, n_capture[i]) for i in idx),
            ...
            "stress_rise_possum_MPa": stress_rise_possum,
            "net_d_tau_MPa": sum(d_tau[i] for i in idx if math.isfinite(d_tau[i])),
            "peak_to_valley_stress_drop_MPa": peak_to_valley_drop,
            "max_n_depin_step": max([max(0.0, n_depin[i]) for i in idx] or [0.0]),
            "max_d_eps_p_step": max([max(0.0, d_eps_p[i]) for i in idx] or [0.0]),
            "min_d_tau_step_MPa": min([d_tau[i] for i in idx if math.isfinite(d_tau[i])] or [float("nan")]),
            "max_rate_dt": max(rdts) if rdts else float("nan"),
            "n_live_pins_median": median([n_live[i] for i in idx]),
        })

    event_av_rows = [a for a in av_rows if a["event_size_depin"] >= min_events]
    total_depin = sum(max(0.0, v) for v in n_depin)
    event_sizes = [a["event_size_depin"] for a in event_av_rows]
    plastic_sizes = [a["plastic_size"] for a in av_rows]
    stress_drops = [a["stress_drop_negsum_MPa"] for a in av_rows]
    depin_step_vals = [max(0.0, v) for v in n_depin]

    top_n = max(1, int(math.ceil(0.01 * len(depin_step_vals))))
    top_fraction = sum(sorted(depin_step_vals, reverse=True)[:top_n]) / total_depin if total_depin > 0 else float("nan")

    win_counts = [sum(depin_step_vals[i:i + window_steps]) for i in range(0, len(depin_step_vals), window_steps)]
    mean_win = sum(win_counts) / len(win_counts) if win_counts else 0.0
    var_win = sum((x - mean_win) ** 2 for x in win_counts) / len(win_counts) if win_counts else 0.0
    fano = var_win / mean_win if mean_win > 0 else float("nan")

    depin_indices = [i for i, v in enumerate(n_depin) if v > 0]
    waits = [steps[j] - steps[i] for i, j in zip(depin_indices[:-1], depin_indices[1:])]
    wait_cv = float("nan")
    if len(waits) >= 2:
        mw = sum(waits) / len(waits)
        vw = sum((w - mw) ** 2 for w in waits) / len(waits)
        wait_cv = math.sqrt(vw) / mw if mw > 0 else float("nan")

    largest_event = max(event_sizes) if event_sizes else 0.0
    largest_plastic = max(plastic_sizes) if plastic_sizes else 0.0
    largest_drop = max(stress_drops) if stress_drops else 0.0
    largest_event_fraction = largest_event / total_depin if total_depin > 0 else float("nan")

    reasons = []
    if total_depin >= 20 and largest_event >= max(5, 0.05 * total_depin):
        reasons.append("one avalanche contains >=5% of all depinning events")
    if math.isfinite(fano) and fano > 5.0 and total_depin >= 20:
        reasons.append("depin-count Fano factor > 5")
    if math.isfinite(top_fraction) and top_fraction > 0.25 and total_depin >= 20:
        reasons.append("top 1% of steps contain >25% of depinning events")
    if largest_drop > 1.0 and largest_plastic > 5.0 * max(med_deps_total, 1e-300):
        reasons.append("stress-drop/plastic-strain burst detected")

    rdts = finite(rate_dt)
    warnings = []
    if rdts and max(rdts) > 0.2:
        warnings.append("event_resolution_limit_affects_avalanche_statistics")
    if total_depin > 0 and not event_av_rows:
        warnings.append("depinning occurred but no event avalanche exceeded min_events")
    ...
    summary = {
        ...
        "plastic_factor": plastic_factor,
        "valid_history_rows": len(rows),
        "total_depin": total_depin,
        "positive_depin_steps": sum(1 for v in n_depin if v > 0),
        "active_steps": sum(1 for x in active if x),
        "n_avalanches_all_activity": len(av_rows),
        "n_avalanches_event_min": len(event_av_rows),
        "largest_event_avalanche_depin": largest_event,
        "largest_event_avalanche_fraction": largest_event_fraction,
        "largest_plastic_avalanche": largest_plastic,
        "largest_stress_drop_negsum_MPa": largest_drop,
        "event_size_mean": sum(event_sizes) / len(event_sizes) if event_sizes else 0.0,
        "event_size_median": median(event_sizes),
        "event_size_gini": gini(event_sizes),
        "depin_step_gini": gini(depin_step_vals),
        "event_count_window_fano": fano,
        "top_1pct_steps_event_fraction": top_fraction,
        "waiting_time_steps_cv": wait_cv,
        "total_plastic_in_active_steps": sum(max(0.0, dp) for dp, a in zip(d_eps_p, active) if a),
        "median_d_eps_total": med_deps_total,
        "max_rate_dt": max(rdts) if rdts else float("nan"),
        "avalanche_like": bool(reasons),
        "avalanche_like_reasons": reasons,
        "warnings": warnings,
    }

    av_fields = [
        "avalanche_id", "start_step", "end_step", "start_time_s", "end_time_s",
        "start_eps_total", "end_eps_total", "duration_steps", "duration_s", "active_steps",
        "event_size_depin", "capture_size", "plastic_size", "imposed_size", "plastic_over_imposed",
        "stress_drop_negsum_MPa", "stress_rise_possum_MPa", "net_d_tau_MPa",
        "peak_to_valley_stress_drop_MPa", "max_n_depin_step", "max_d_eps_p_step",
        "min_d_tau_step_MPa", "max_rate_dt", "n_live_pins_median",
    ]
    with open(outdir / "avalanche_events.csv", "w", newline="") as f:
        w = _csv.DictWriter(f, fieldnames=av_fields)
        w.writeheader(); w.writerows(av_rows)
    (outdir / "avalanche_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True))
    with open(outdir / "avalanche_summary.csv", "w", newline="") as f:
        w = _csv.DictWriter(f, fieldnames=list(summary.keys()))
        w.writeheader(); w.writerow(summary)

    lines = ["Avalanche diagnostic", "===================="]
    for key in [
        "total_depin", "positive_depin_steps", "active_steps",
        "n_avalanches_all_activity", "n_avalanches_event_min",
        "largest_event_avalanche_depin", "largest_event_avalanche_fraction",
        "largest_plastic_avalanche", "largest_stress_drop_negsum_MPa",
        "event_count_window_fano", "top_1pct_steps_event_fraction",
        "waiting_time_steps_cv", "event_size_gini", "depin_step_gini",
        "max_rate_dt", "avalanche_like",
    ]:
        lines.append(f"{key}: {summary.get(key)}")
    if reasons:
        lines.append("avalanche_like_reasons:")
    ...
    (outdir / "avalanche_summary.txt").write_text("\n".join(lines) + "\n")
...
def write_run_summary(outdir: Path, params: dict, target_strain: float) -> None:
    ...
    summary = {
        ...
        "preflight_pin_survival_prob_one_spacing": params.get("preflight_pin_survival_prob_one_spacing"),
        "v15_cross_floor_frac_used": params.get("v15_cross_floor_frac_used"),
        "v15_peierls_floor_frac_used": params.get("v15_peierls_floor_frac_used"),
    }
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--plastic-strain-source", choices=["actual", "free_glide", "total"], default="actual",
    ...
    ap.add_argument("--no-avalanche-summary", action="store_true",
                    help="disable writing avalanche_summary.{json,txt,csv} and avalanche_events.csv at the end of the run")
    ap.add_argument("--avalanche-quiet-steps", type=int, default=2,
                    help="group active depinning/plastic-burst steps into one avalanche if separated by no more than this many quiet steps")
    ap.add_argument("--avalanche-window-steps", type=int, default=100,
                    help="window length in steps for depinning-count Fano-factor burst diagnostic")
    ap.add_argument("--avalanche-min-events", type=int, default=2,
                    help="minimum total depinning events for an event-defined avalanche")
    ap.add_argument("--avalanche-plastic-factor", type=float, default=1.0,
                    help="also mark a step active when d_eps_p exceeds this factor times the median imposed strain increment")
    ...
    # v14 preflight: cheap kinetic/time-scale diagnostics before the expensive run.
    ...
    params.update({
        ...
        "model": "clean_arrhenius_taylor_explicit_obstacles_v16_branch_floor_avalanche",
    ...
    with open(outdir / "single_glider_history.csv", "w", newline="") as hf, \
        ...
        for step in range(1, nsteps + 1):
            ...
            # v13: plastic strain from actual swept line area by default.
            #
            ...
            hw.writerow({
                ...
                "tau_before_step_MPa": tau_before_step_MPa,
    ...
    if not args.no_avalanche_summary:
        try:
            write_avalanche_summary(outdir, params, args.target_strain)
        except Exception as exc:
            (outdir / "avalanche_summary.error.txt").write_text(str(exc) + "\n")
