...
def main(argv=None) -> int:
    ...
    ap.add_argument("--max-free-dx-reduced", type=float, default=2.0,
                    help="v5: pure CFL clip on |dx| per step (do not skip past obstacles); "
                         "NOT a glide prefactor anymore. Set ~capture radius.")
    ...
    with open(outdir / "single_glider_history.csv", "w", newline="") as hf, \
        ...
        for step in range(1, nsteps + 1):
            ...
            dx_all = np.clip(dx_trial, -args.max_free_dx_reduced, args.max_free_dx_reduced)
            ...
            for key, j in list(pinned_pair_node.items()):
                ...
                if age_steps >= args.min_pin_age_steps and rng.random() < p_event:
            ...
            hw.writerow({
                ...
                "crossing_barrier_model": "expfit_floor_direct_v5_signed",
