...
def line_tension_back_stress_MPa(x_lines: np.ndarray, line_tension_N: float, b_m: float,
    ...
    positive tau_back that opposes glide.  We clip to [0, cap] so line tension
    only ever RESISTS the advance (it never drives glide beyond tau_app), and
    so a single stiff step cannot overshoot.  Minimum-image handles the x
    periodicity and the periodic z-closure (uniform dz_node spacing).
    """
    ...
    return np.clip(tau_back_MPa, 0.0, float(cap_MPa))
...
def main(argv=None) -> int:
    ...
    with open(outdir / "single_glider_history.csv", "w", newline="") as hf, \
        ...
        for step in range(1, nsteps + 1):
            ...
            # v4: each free node glides under its LOCAL effective stress
            #     tau_eff,j = max(0, tau_app - tau_back,j),  tau_back = (T/b)*kappa_j.
            # As a segment bows against its pins, kappa (hence tau_back) grows, tau_eff
            # falls, and the segment slows; at equilibrium tau_back = tau_app -> the
            # segment arrests, held back by the pins.  After a depin the local curvature
            # relaxes, tau_back collapses, and the segment springs forward.  This is the
            # force balance along the bowed segment, and it uses the SAME line tension T
            # as the pin reaction force.  Set --backstress-mobility off to recover v3.
            free = ~pinned
            if args.backstress_mobility == "on":
                ...
                tau_eff = np.maximum(0.0, tau_MPa - tau_back_MPa)
                Gp_arr = peierls.DeltaG_eV_vec(tau_eff, T)
                rate_arr = args.attempt_frequency_s * args.mobile_glide_prefactor * np.exp(
                    -Gp_arr / max(KB_EV * T, 1e-300))
                dx_all = np.minimum(args.max_free_dx_reduced,
                                    args.max_free_dx_reduced * rate_arr * args.dt)
            else:
                Gp = peierls.DeltaG_eV(tau_MPa, T)
                rate_p = args.attempt_frequency_s * args.mobile_glide_prefactor * math.exp(
                    -Gp / max(KB_EV * T, 1e-300))
                dx_all = np.full_like(x_lines, min(args.max_free_dx_reduced,
                                                   args.max_free_dx_reduced * rate_p * args.dt))

            dx_free = np.where(free, dx_all, 0.0)
            v_free = float(np.mean(dx_free[free]) / args.dt) if np.any(free) else 0.0
            ...
            # v3: pin ONLY the single nearest node per (line, obstacle); do NOT snap it
            # to obs_x (pin at its current x).  Other nodes within the capture radius of
            # the same obstacle stay FREE and only contribute to the pile-up count.
            # Pile-up is recomputed fresh every step (geometric), never accumulated.
            n_capture = 0
            n_candidate_tests = 0
            ...
            if nobs > 0 and args.capture_radius_reduced > 0:
                ...
                for li in range(nline):
                    # node -> nearest unpassed obstacle within radius
                    ...
                    for j in range(nn):
                        ...
                        chosen = None
                        bestd = None
                        for oid0 in cands[np.argsort(d2[cands])]:
                            oid = int(oid0)
                    ...
                    for oid, lst in targets.items():
                        ...
                        if key in pinned_pair_node:
                            ...
                            # nearest node becomes the lead junction (no snap), rest pile up
                            _, jlead = lst[0]
            ...
            for key, j in list(pinned_pair_node.items()):
                ...
                # v3: direct force-per-area concentration (monotone, cannot saturate at
                # the tau*v* peak).  Legacy implicit solve kept behind --tau-local-mode.
                if args.tau_local_mode == "direct":
                    ...
                    work_eV = tau_local * 1.0e6 * vstar / EV_J
                else:
